...
>(
  (
    ...
    const [assertiveMessage, setAssertiveMessage] =
      React.useState("");
    const prevSelectedCount = React.useRef(
    ...
    const prevDefaultValueRef =
      React.useRef<string[]>(defaultValue);
    ...
    const getResponsiveSettings = () => {
      ...
      if (responsive === true) {
        ...
        return {
          ...
          compactMode:
            currentSettings?.compactMode ?? false,
        };
      }
    ...
    const getAllOptions =
      React.useCallback((): MultiSelectOption[] => {
        if (options.length === 0) return [];
        let allOptions: MultiSelectOption[];
        if (isGroupedOptions(options)) {
          allOptions = options.flatMap(
            (group) => group.options
          );
        } else {
          allOptions = options;
    ...
    const toggleAll = () => {
      ...
      if (selectedValues.length === allOptions.length) {
        ...
        const allValues = allOptions.map(
          (option) => option.value
        );
    ...
    React.useEffect(() => {
      ...
      if (selectedCount !== prevSelectedCount.current) {
        const diff =
          selectedCount - prevSelectedCount.current;
        if (diff > 0) {
      ...
      <>
        ...
        >
          <div
            ...
            className="sr-only"
          >
            Multi-select dropdown. Use arrow keys to
